...
impl Parser {
    ...
    pub fn parse<'source>(&mut self, source_code: &'source str) -> Option<Tree<'source>> {
        self.parser.parse(&source_code, None).map(|tree| convert_to_tree(tree.root_node(), source_code))
    }
}
...
fn convert_to_tree<'source>(node: tree_sitter::Node, source_code: &'source str) -> Tree<'source> {
    Tree {
        ...
        children: (0..node.child_count()).map(|i| convert_to_tree(node.child(i).unwrap(), source_code)).collect(),
        range: node.range(),
        source: source_code,
    }
}
